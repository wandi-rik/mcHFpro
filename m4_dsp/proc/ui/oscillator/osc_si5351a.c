...
static uint16_t Si5351a_WriteRegisters(uint8_t reg, uint8_t* val_p, uint32_t size)
{
	...
	return 	mchf_hw_i2c_WriteBlock(SI5351_I2C_WRITE, reg, 1, val_p, size);
#endif
...
static bool Si5351a_SetupPLL(uint8_t pll, uint8_t mult, uint32_t num, uint32_t denom)
{
	...
	return Si5351a_WriteRegisters(pll, pll_data, 8) == 0;// HAL_OK;
}
...
static bool Si5351a_SetupMultisynthInteger(uint8_t synth, uint32_t divider, uint8_t rDiv)
{
	...
	return Si5351a_WriteRegisters(synth, synth_data, 8) == 0;//HAL_OK;
}
...
//!	si5351a_state.is_present = MCHF_I2C_DeviceReady(SI5351A_I2C,SI5351_I2C_WRITE) == 0;//HAL_OK;
