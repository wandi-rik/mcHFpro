...
uchar				share_port  = 0;
//ResMgr_Status_t 	mngr_status = RESMGR_ERROR;
//ResMgr_Status_t 	port_status = RESMGR_OK;
...
//*----------------------------------------------------------------------------
static void claim_debug_port(void)
{
	if(share_port == 0)
		return;

	//ulong timeout = 0xFFFF;
	...
	//do
	//{
	//	port_status = ResMgr_Request(RESMGR_ID_USART1, RESMGR_FLAGS_ACCESS_NORMAL|RESMGR_FLAGS_CPU1 , 0, NULL);
	//	timeout--;
	//}while ((port_status == RESMGR_BUSY) && (timeout));
...
signed int printf(const char *pFormat, ...)
{
    ...
    claim_debug_port();
    HAL_UART_Transmit(&DEBUG_UART_Handle, (uchar *)cpu_id_str, sizeof(cpu_id_str), 0xFFFF);
...
void printf_init(uchar is_shared)
{
	...
	{
		// Init
		//mngr_status = ResMgr_Init(ResMgrSendMessage, ResMgrCallback);
