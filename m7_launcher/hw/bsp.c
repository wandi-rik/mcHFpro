...
//extern void BSP_ConfigSerial(void);
...
uint8_t BSP_Config(void)
{
  ...
  printf_init(0);
  printf("-----------------------------------------\r\n");
  printf("--- Debug Print Session on (launcher) ---\r\n");
  printf("Firmware v: %d.%d.%d.%d\r\n", MCHF_L_VER_MAJOR, MCHF_L_VER_MINOR, MCHF_L_VER_RELEASE, MCHF_L_VER_BUILD);
...
void BSP_JumpToSubDemo(uint32_t SubDemoAddress)
{
	...
	printf("Jumping to radio firmware at 0x%08x ...\r\n", (int)SubDemoAddress);
