...
#define ID_LISTBOX1						(GUI_ID_USER + 0x39)
...
static const GUI_WIDGET_CREATE_INFO _aMainDialogCreate[] = {
  ...
  // Test
  //{ TEXT_CreateIndirect   , "400 MHz"             , ID_TEXT_CPU_FREQ_VAL  , 10 , 10 , 250 ,  30 , (GUI_TA_LEFT | GUI_TA_TOP)     , 0x0, 0 },
  //
  { LISTBOX_CreateIndirect, "BandActivity.Listbox",		ID_LISTBOX1, 		400, 		23, 	390, 	200, 	0, 		0x0, 	0 },
};
...
static const GUI_WIDGET_CREATE_INFO _aCalibrationCreate[] = {
	...
	//
    //{ BUTTON_CreateIndirect, "Top_Left", 	ID_BUTTON_TOP, 			0, 		380,	100, 	100, 0, 0x0, 0 },
    //{ BUTTON_CreateIndirect, "Bot_right", 	ID_BUTTON_BOTTOM, 		700, 	380, 	100, 	100, 0, 0x0, 0 },

	{ LISTBOX_CreateIndirect, 	"Listbox",	ID_LISTBOX1, 			5, 		5, 		790, 	445, 	0, 		0x0, 	0 },
	//
	{ HEADER_CreateIndirect,	"Header", 	ID_HEADER_0,			5, 		450, 	790, 	30, 	0, 		0x0, 	0 },
	{ PROGBAR_CreateIndirect,	"Progbar", 	ID_PROGBAR_0,			500,	453,	294, 	21, 	0,		0x0, 	0 },
};
/*********************************************************************
*
*       Static code
...
static void _cbMainDialog(WM_MESSAGE * pMsg)
{
  ...
  case WM_INIT_DIALOG:
    ...
    LISTBOX_AddString(hList, "network 4"   );
    LISTBOX_AddString(hList, "network 5"   );
    LISTBOX_AddString(hList, "network 6"   );
    LISTBOX_AddString(hList, "network 7"   );
...
void ml_periodic(void)
{
	...
	hItem = WM_GetDialogItem(hWinBase, ID_TEXT_CPU_FREQ_VAL);

	//sprintf((char *)tmp , "CM7 %lu MHz, CM4 %lu MHz", (HAL_RCCEx_GetD1SysClockFreq()/1000000U), (HAL_RCC_GetHCLKFreq()/1000000U));
	sprintf((char *)tmp , "%d", (int)i++);
...
static void state_machine(WM_MESSAGE * pMsg)
{
	...
	switch(state_id)
	{
		...
		{
			hList = WM_GetDialogItem(pMsg->hWin, ID_LISTBOX1);
			LISTBOX_AddString(hList, "Reading ESP32 co-processor firmware version..." );
			//LISTBOX_SetSel(hList, -1);
		...
		case 23:
		{
			...
			{
				LISTBOX_AddString(hList, "Reading ESP32 co-processor firmware version...Success" );
			...
			{
				memset(buf, 0, sizeof(buf));
				sprintf(buf,  "Reading ESP32 co-processor firmware version...Error %d", esp_msg.ucExecResult);
				LISTBOX_AddString(hList, buf);
			}
			//LISTBOX_SetSel(hList, -1);
			last_item++;
		...
		{
			hList = WM_GetDialogItem(pMsg->hWin, ID_LISTBOX1);
			LISTBOX_AddString(hList, "Reading WiFi connection details..." );
			//LISTBOX_SetSel(hList, -1);
		...
		case 25:
		{
			...
			if(esp_msg.ucExecResult == 0)
			{
				LISTBOX_AddString(hList, "Reading WiFi connection details...Success" );
				if(esp_msg.ucDataReady)
				{
			...
			{
				memset(buf, 0, sizeof(buf));
				sprintf(buf,  "Reading WiFi connection details...Error %d", esp_msg.ucExecResult);
				LISTBOX_AddString(hList, buf);
			}
			//LISTBOX_SetSel(hList, -1);
			last_item++;
...
static void _cbCalibrationScreen(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		case WM_INIT_DIALOG:
		{
			...
			//hScrollV = SCROLLBAR_CreateAttached(hList, SCROLLBAR_CF_VERTICAL);
			//SCROLLBAR_SetColor(hScrollV,SCROLLBAR_CI_THUMB|SCROLLBAR_CI_SHAFT|SCROLLBAR_CI_ARROW,GUI_BLUE);
			//SCROLLBAR_SetWidth(hScrollV,30);
			...
			//PROGBAR_SetBarColor(hItem, 1, GUI_LIGHTBLUE);
			PROGBAR_SetMinMax(hItem, 0,100);
			//PROGBAR_SetTextColor(hItem, 0, GUI_MAKE_COLOR(GUI_PROGBAR_DARKBLUE));
			//PROGBAR_SetTextColor(hItem, 1, GUI_MAKE_COLOR(GUI_PROGBAR_DARKBLUE));

			hItem = WM_GetDialogItem(pMsg->hWin, ID_HEADER_0);
			HEADER_SetFont(hItem, &GUI_Font24B_1);
			//HEADER_SetTextAlign(hItem, 0, GUI_TA_HCENTER);	// ToDo: Doesn't work ;(
			memset(fw_id,0,sizeof(fw_id));
			sprintf(fw_id,"%d.%d.%d.%d", MCHF_L_VER_MAJOR, MCHF_L_VER_MINOR, MCHF_L_VER_RELEASE, MCHF_L_VER_BUILD);
			HEADER_AddItem(hItem, 120, fw_id, 12);
			HEADER_AddItem(hItem, 220, " ... ", 12);
			HEADER_AddItem(hItem, 60, "", 12);

			// Request UART call
			esp_msg.ucMessageID = 0x03;				// connect to network
			esp_msg.ucProcStatus = TASK_PROC_WORK;
			osMessagePut(hEspMessage, (ulong)&esp_msg, osWaitForever);
		...
		case WM_PAINT:
		{
			...
			// Os Usage
			hItem = WM_GetDialogItem(pMsg->hWin, ID_HEADER_0);
			HEADER_DeleteItem(hItem, 2);
			sprintf(fw_id,"%04x",(int)cnt++);
			HEADER_AddItem(hItem, 60, fw_id, 12);

			if(state_id < 20) state_id++;
...
void LauncherStartUp(const uint8_t SoftwareReset)
{
    ...
    // Execute radio firmware
	#if 1
    if(*(ulong *)0x08080004 != 0xFFFFFFFF)
    {
    	// IRQ off
    	portDISABLE_INTERRUPTS();
    	// Jump
    	BSP_JumpToSubDemo(0x08080000);
    }
	#endif
